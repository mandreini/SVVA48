...
def normal_stress_calculator(Mx, My, epsilon0=(2,2)):
    #Create booms
    epsilon, epsilon2 = epsilon0
    ...
    #ITERATION
    # for i in range(20):
    while epsilon > 0.01 and epsilon2 > 0.01:
...
def shear_stress_calculator(all_booms, Vx, Vy, Torque):
    ...
    q01, q02 = constant_shear_flow_calculator(cell1, cell2, Torque)

    q1,fx1,fy1,cell1 = calculate_opensection_shear_flows(cell1,Vx,Vy,Ixx_cell1,Iyy_cell1,centroid_cell1,-2)
    q2,fx2,fy2,cell2 = calculate_opensection_shear_flows(cell2,Vx,Vy,Ixx_cell2,Iyy_cell2,centroid_cell2,-2)

    qs1 = q1 + q01
    qs2 = q2 + q02
    qq = make_q_into_matrix(cell1, qs1, cell2, qs2)
    return qq


def make_q_into_matrix(cell1, q1, cell2, q2):
    cell1_q = numpy.roll(numpy.column_stack((cell1, q1)), -1, axis=0)
    last_cell1q = cell1_q[-3:]
    cell2_q = numpy.roll(numpy.column_stack((cell2, q2)), -1, axis=0)
    floor2 = cell2_q[cell2_q[:, 2] == -parameters.R + parameters.hf]
    floor1 = cell1_q[cell1_q[:, 2] == -parameters.R + parameters.hf]
    floor_q = floor1[:, 3] + floor2[:, 3]
    # floor1[:3] = floor_q
    # floor1[:,3] = floor_q
    total_fueslage_q = numpy.append(cell1_q[0:-8], cell2_q[5:], axis=0)
    total_fueslage_q = numpy.append(total_fueslage_q, last_cell1q, axis=0)
    total_fueslage_q = numpy.append(total_fueslage_q, floor1, axis=0)

    return total_fueslage_q
